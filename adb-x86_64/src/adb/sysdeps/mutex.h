...
#if (defined(_WIN32) && !(defined(__MINGW64__) || defined(__MINGW32__))) || !defined(_GLIBCXX_HAS_GTHREADS)
...
class mutex {
  public:
    ...
    void unlock() {
        if (--lock_count_ != 0) {
            fatal("non-recursive mutex unlock resulted in unexpected lock count: %d", (int)lock_count_);
        }
        mutex_.unlock();
    }
